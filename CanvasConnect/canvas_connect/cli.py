...
def build_parser() -> argparse.ArgumentParser:
    ...
    inspect_parser.add_argument("--export", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "inspect-export":
        inspect_export_step(args.export, args.run_dir)
        return 0
    ...
    if args.command == "review-matches":
        ...
        roster = args.roster or args.run_dir / "roster" / "roster.json"
        review_matches_step(manifest, roster, args.run_dir, assume_yes=args.yes)
        return 0
